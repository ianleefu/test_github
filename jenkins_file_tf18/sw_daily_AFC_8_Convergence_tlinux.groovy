...
timeout(20){util_make.load_kmd("${asic_config}")}
...
def myImage = docker.image('b5c3d3dd727a')
myImage.inside('--user root --network host --privileged -v /data/:/home/workspace/ -v /root/.ssh:/root/.ssh/ --device=/dev/dtu0:/dev/dtu0'){
    ...
    try{
        ...
        util_test.resnet_train_test("${batch}", "${epoch}", "${training_once}", "${dtype}", "${data_format}", "${test_env}")
    }
    catch(e){
