...
try{
    ...
    //object_detection model added, test script stored in floder "benchmark/enflame_object_detection".
    if( model_type == "object_detection"){
        util_test.object_detection_train_test("${batch}", "${epoch}", "${step}", "${dtype}", "${data_format}", "${test_env}", "${model_name}", "${dataset}","2","${repeat_time}")
        util_log.get_fps_result("${env.WORKSPACE}")
    }
    else if (model_type == "minibenchmark"){
        util_test.minibenchmark_test("${batch}", "${epoch}", "${dtype}", "${data_format}", "${test_env}", "2", "${device}", "${net_size}","${repeat_time}" )
    }
    else{
        util_test.resnet_train_test("${batch}", "${epoch}", "${step}", "${dtype}", "${data_format}", "${test_env}", "${model_name}", "${dataset}","2","${repeat_time}")
        util_log.get_fps_result("${env.WORKSPACE}")
    }
}
catch(e){
